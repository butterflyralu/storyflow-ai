# Epic Update Plan: Clone Chat History per Sub-Story & Improve Epic Naming
...
### 1. Update `usePersistedChat.ts` — Add `cloneSession`
- Add a `cloneSession` method that creates a new session and copies all messages from an existing session into it
- The new session gets a title matching the child story's title

### 2. Enhance `createEpic` in `src/hooks/useStorySaver.ts`
...
### 3. Enhance `saveEpicWithStories` in `src/hooks/useStorySaver.ts`
- For each child story, **create a new chat session** (via `usePersistedChat.cloneSession`) with a title matching the child story's title
...
- Build a rich description for the epic that includes the original story's full details

### 4. Update `SplitStoriesView.tsx` → `handleSaveAll`
