...
function StoryCard({
  ...
  story: StoryDraft; index: number; total: number;
  onUpdate: (update: Partial<StoryDraft>) => void;
  onRemove: () => void; onSave: () => void; isSaved: boolean;
  sessionId: string; contextId: string | null;
}) {
  ...
  const handleEvaluate = async () => {
    setEvaluating(true);
    try {
      const result = await api.evaluateStory({ story, sessionId, contextId: contextId || '' });
    ...
    <>
      <Card className="flex h-full flex-col border shadow-md">
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              ...
              <Button size="icon" variant="ghost" className="h-7 w-7 text-muted-foreground hover:text-destructive"
                onClick={() => total > 1 ? setShowRemoveConfirm(true) : toast({ title: 'Cannot remove', description: 'You need at least one story.' })}>
                <Trash2 className="h-3.5 w-3.5" />
              </Button>
            </div>
          </div>
          <input value={story.title} onChange={e => onUpdate({ title: e.target.value })}
          ...
          {story.metadata?.epic && (
            <Badge variant="secondary" className="mt-1 text-xs">Epic: {story.metadata.epic}</Badge>
          )}
        </CardHeader>
        <CardContent className="flex-1 space-y-3 overflow-y-auto">
          <div className="rounded-lg border border-border bg-muted/20 p-3 space-y-1 text-sm">
            <div className="flex items-baseline gap-1.5">
              ...
              <input value={story.asA} onChange={e => onUpdate({ asA: e.target.value })}
                className="flex-1 border-0 bg-transparent text-sm text-foreground placeholder:text-muted-foreground focus:outline-none" placeholder="[role]" />
            </div>
            <div className="flex items-baseline gap-1.5">
              <span className="font-semibold text-foreground">I want to</span>
              <input value={story.iWant} onChange={e => onUpdate({ iWant: e.target.value })}
                className="flex-1 border-0 bg-transparent text-sm text-foreground placeholder:text-muted-foreground focus:outline-none" placeholder="[capability]" />
            </div>
            <div className="flex items-baseline gap-1.5">
              <span className="font-semibold text-foreground">So that</span>
              <input value={story.soThat} onChange={e => onUpdate({ soThat: e.target.value })}
                className="flex-1 border-0 bg-transparent text-sm text-foreground placeholder:text-muted-foreground focus:outline-none" placeholder="[value]" />
            </div>
          </div>
          <div>
            <div className="mb-1 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Description</div>
            <textarea value={story.description} onChange={e => onUpdate({ description: e.target.value })}
              ...
              placeholder="Story description..." rows={2} />
          </div>
          <div>
        ...
        <div className="border-t border-border px-4 py-2 flex gap-2">
          <Button size="sm" variant="outline" className="flex-1 gap-1 text-xs" onClick={handleEvaluate} disabled={evaluating}>
            {evaluating ? <Loader2 className="h-3 w-3 animate-spin" /> : <ClipboardCheck className="h-3 w-3" />}
            {evaluating ? 'Evaluating…' : 'Evaluate'}
          </Button>
          <Button size="sm" className="flex-1 gap-1 text-xs" onClick={onSave} disabled={isSaved}>
      ...
      <AlertDialog open={showRemoveConfirm} onOpenChange={setShowRemoveConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will remove "{story.title || `Story ${index + 1}`}" from the split. This cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={() => { setShowRemoveConfirm(false); onRemove(); }} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
              Remove
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
...
export function SplitStoriesView() {
  const {
    splitStories, activeSplitIndex, setActiveSplitIndex, updateSplitStory,
    removeSplitStory, epicSummary, clearSplit, saveStory, addMessage,
    story: originalStory, sessionId, contextId, dbSessionId, triggerSidebarRefresh,
  } = useWizard();
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      if (result) {
        ...
        splitStories.forEach((s, i) => { if (!savedIndices.has(i)) saveStory(s); });
    ...
    <div className="flex h-full flex-col">
      <div className="sticky top-0 z-10 border-b border-border bg-card/95 backdrop-blur-sm px-5 py-3">
        <div className="flex items-center justify-between">
          <div className="flex-1 min-w-0">
            ...
            {epicSummary && <p className="text-sm text-muted-foreground truncate">{epicSummary}</p>}
          </div>
          <Button size="sm" variant="ghost" onClick={() => setShowDiscardConfirm(true)} className="ml-3 gap-1.5 text-xs">
      ...
      <div className="flex-1 px-10 py-5">
        <Carousel
          ...
          className="w-full"
          setApi={(api) => {
            api?.on('select', () => { setActiveSplitIndex(api.selectedScrollSnap()); });
          }}
        >
          <CarouselContent>
            {splitStories.map((s, i) => (
              <CarouselItem key={i} className="basis-[85%] md:basis-[75%]">
                <div className="h-[calc(100vh-220px)]">
                  <StoryCard story={s} index={i} total={splitStories.length}
        ...
        <div className="flex items-center justify-center gap-1.5 mt-4">
          {splitStories.map((_, i) => (
            <button key={i}
              className={`h-2 w-2 rounded-full transition-colors ${i === activeSplitIndex ? 'bg-primary' : 'bg-muted-foreground/30'}`}
      ...
      <div className="border-t border-border bg-card px-5 py-3 flex gap-2">
        ...
        <Button variant="secondary" onClick={() => setShowDiscardConfirm(true)} className="gap-1.5">
          <X className="h-4 w-4" />
          Discard Split
        </Button>
      </div>
      ...
      <AlertDialog open={showDiscardConfirm} onOpenChange={setShowDiscardConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Discard split stories?</AlertDialogTitle>
            <AlertDialogDescription>
              This will discard all {splitStories.length} split stories and return to the original story. Unsaved changes will be lost.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={() => { setShowDiscardConfirm(false); clearSplit(); }}>
              Discard
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
